...
Brain fluid dynamics governs waste clearance, impacting neurological health. Quantifying these flows, particularly deep within tissue, remains challenging. Here, we introduce Magnetic Resonance Artificial Intelligence Velocimetry (MR-AIV), a computational approach constrained by porous media flow physics, using \textit{in vivo} dynamic imaging to map 3D velocity fields throughout the mouse brain. MR-AIV integrates advanced denoising and physics-informed techniques to robustly handle experimental noise, revealing consistent patterns of slow parenchymal flows ($\sim$ 0.1 $\mu$ m/s), distinct from faster perivascular pathways ($\sim$ 3 $\mu$ m/s). Transport analysis identifies spatially segregated mechanisms: diffusion dominates in the parenchyma while advection governs specific high-flow routes. This quantitative characterization, including rigorous velocity uncertainty estimates, provides a powerful tool to investigate glymphatic physiology and its disruption in disease, enabling more accurate models of brain transport. The method's performance is evaluated using synthetic datasets, showing good agreement with the ground truth.
